...
- [ ] Google Search Console にドメインプロパティとして登録（ブログは登録済み）。
      サイトマップの送信は必須ではない —— `robots.txt` に `Sitemap:` を書いてあるので
      放っておいても見つかる。登録の値打ちは送信ではなく、301 が辿られているかや
      404 が増えていないかを見られること
- [x] X でトップページを共有し、OGP の見え方を確認
...
このサイトは第三者のスクリプトを一切読み込まない。外部への参照はすべて
ただのリンクで、訪問者が押すまで何も送られない。`/policy/` に個人情報や
Cookie の節が無いのはそのためで、書くべきことが無いから書いていない。

blog.kazuki.page は Google アナリティクスを入れているので、あちらのポリシーには
その記載がある。**2 つのポリシーで粒度が違うのは実態が違うからで、揃える必要はない。**

Search Console はこれに当たらない。サイト側でスクリプトが動かず、見えるのは
Google が検索側で持っているデータなので、訪問者のデータを集めていない。

**あとからアナリティクスを入れるなら、そのときは `/policy/` に追記が要る。**
